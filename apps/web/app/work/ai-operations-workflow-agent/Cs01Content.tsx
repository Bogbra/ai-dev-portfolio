...
const c = {
  en: {
    ...
    archSteps: [
      { num: '01', title: 'Upload & validate', description: 'Next.js sends the file as a base64 JSON payload to Python/FastAPI. The backend validates file extension, size, structure and parser validity, then parses CSV or XLSX. Required columns: name, email. Data is normalised and returned as a typed array — never written to disk.' },
    ...
    techImpl: [
      ...
      { title: 'Rate limiting', items: ['SlowAPI (Python/FastAPI) per route: 5 runs/hour, 20 uploads/hour', 'The Node/Fastify contact form service has its own separate 100 req/min limit — the AI service uses only real per-route SlowAPI limits, no shared global default', 'Rate limit responses expose safe messages without internal details', 'Per-IP limits are in-memory per process — correct for this single-instance deployment; multiple replicas would need a shared store'] },
      { title: 'File ingestion', items: ['Client sends file as base64 JSON payload — avoids multipart complexity', 'Python/FastAPI decodes, validates size and structure, then parses with openpyxl/csv', 'normalise_contacts() sanitises all fields and validates email format with Pydantic', 'Contacts returned as a typed array to Next.js — never stored on disk', 'Both CSV and XLSX parsed through a unified normalisation pipeline'] },
  ...
  de: {
    ...
    archSteps: [
      { num: '01', title: 'Upload & Validierung', description: 'Next.js sendet die Datei als Base64-JSON-Payload an Python/FastAPI. Das Backend validiert Dateiendung, Größe, Struktur und Parser-Ergebnis, parst CSV oder XLSX und normalisiert die Kontakte. Erforderliche Spalten: Name, E-Mail. Daten werden nie auf Datenträger gespeichert.' },
