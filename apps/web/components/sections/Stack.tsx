...
function StageCard({
  ...
  return (
    <motion.div
      ...
      animate={isVisible ? { opacity: 1, y: 0 } : { opacity: 0, y: prefersReduced ? 0 : 20 }}
      whileHover={prefersReduced ? {} : { scale: 1.015 }}
      transition={{
        duration: prefersReduced ? 0 : 0.5,
        ease: easings.outExpo,
        delay: prefersReduced ? 0 : 0.1 + index * 0.07,
    ...
    <section id="stack" className="py-24 md:py-32 px-8 md:px-16 lg:px-20 bg-surface">
      <div className="max-w-[1920px] mx-auto">
        ...
        <motion.p
          ...
          transition={{ duration: prefersReduced ? 0 : 0.4, delay: prefersReduced ? 0 : 0.8 }}
          aria-hidden="true"
        >
          {t.stack.flow}
        </motion.p>
